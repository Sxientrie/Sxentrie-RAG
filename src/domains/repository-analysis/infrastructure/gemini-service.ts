...
const fetchFileContents = async (files: GitHubFile[], config: AnalysisConfig): Promise<string> => {
...
export const runCodeAnalysis = async (
  ...
  if (config.scope === ANALYSIS_SCOPES.FILE && selectedFile && !selectedFile.isImage) {
    ...
    const filesToFetchCount = config.mode === ANALYSIS_MODES.FAST
      ? Math.min(allFiles.length, MAX_GEMINI_FILE_COUNT)
      : allFiles.length;
    onProgress(`Fetching content for ${filesToFetchCount} files`);
    fileContentsString = await fetchFileContents(allFiles, config);
