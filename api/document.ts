  ...
  const fileContents = await Promise.all(
    files.map(async (file) => {
      ...
      try {
        ...
        const cappedText = text.length > MAX_GEMINI_FILE_SIZE
          ? text.substring(0, MAX_GEMINI_FILE_SIZE) + TRUNCATED_GEMINI_MESSAGE
          : text;
...
export default async function handler(request: ServerlessRequest) {
  try {
    ...
    // Note: Documentation generation doesn't need progress updates for fetching yet,
    // so we can do it in one batch. This can be upgraded to streaming later if needed.
    ...
    const stream = new ReadableStream({
        async start(controller) {
            ...
            for await (const chunk of docStream) {
                // FIX: The `.text` property on a stream chunk is a getter, not a method.
